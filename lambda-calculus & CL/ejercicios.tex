...
\begin{enumerate}
  \renewcommand{\labelenumi}{\alph{enumi})}
  \item $\mathrm{AP} \vdash \forall x \,(x \leq x)$
  \item $\mathrm{AP} \vdash \forall x \forall y \,((x \leq y \wedge y \leq x) \rightarrow x = y)$
  \item $\mathrm{AP} \vdash \forall x \forall y \forall z \,((x \leq y \wedge y \leq z) \rightarrow (x \leq z))$
  \item $\mathrm{AP} \vdash \forall x \,(0 \leq x)$
  \item $\mathrm{AP} \vdash \forall x \forall y \,(x \leq y \vee y \leq x)$
  \item $\mathrm{AP} \vdash \forall x \forall y \,(x < y \vee x = y \vee y < x)$
  \item $\mathrm{AP} \vdash \forall x \forall y \forall z \,(x \leq y \rightarrow x + z \leq y + z)$
\end{enumerate}
\end{theorem}
...
\item Propiedades de las fórmulas de desigualdad: 
\[\textbf{AP} \vdash \forall x \forall y (x < sy \leftrightarrow x \leq = y)\]
\[\textbf{AP} \vdash \forall x \forall y (x < y \leftrightarrow sx \leq = y)\]
\item Propiedades de la divisibilidad:
\[\textbf{AP} \vdash \forall x \forall y \forall z \leq y (x \mid y \land x \mid z \rightarrow \mid y \pm z)\]
...
\[\textbf{AP} \vdash \forall x \forall y \forall k \,(k \mid x \land coprimos(x,y) \rightarrow (k, y))\]
...
\paragraph{Solucion}La solución de Gödel consiste en representar cualquier fórmula finita de números naturales mediante un único número natural. Para ello, se define una función inyectiva para identificar cada fórmula de nuestra teoría: 
\[
...
+ & 2^{2} & t_0 + t_1 & 2^{\#t_0} \cdot 3^{\#+} \cdot 5^{\#t_1} & \varphi_0 \land \varphi_1 & 2^{\#\land} \cdot 3^{\#\varphi_0} \cdot 5^{\#\varphi_1} \\
\cdot & 2^{3} & t_0 \cdot t_1 & 2^{\#t_0} \cdot 3^{\#\cdot} \cdot 5^{\#t_1} & \varphi_0 \lor \varphi_1 & 2^{\#\lor} \cdot 3^{\#\varphi_0} \cdot 5^{\#\varphi_1} \\
= & 2^{4} &  &  & \varphi_0 \rightarrow \varphi_1 & 2^{\#\rightarrow} \cdot 3^{\#\varphi_0} \cdot 5^{\#\varphi_1} \\
\land & 2^{5} &  &  & \exists x\,\varphi & 2^{\#\exists} \cdot 3^{\#x} \cdot 5^{\#\varphi} \\
\lor & 2^{6} &  &  & \forall x\,\varphi & 2^{\#\forall} \cdot 3^{\#x} \cdot 5^{\#\varphi} \\
\rightarrow & 2^{7} &  &  &  &  \\
\exists & 2^{8} &  &  &  &  \\
\forall & 2^{9} &  &  &  &  \\
\hline
